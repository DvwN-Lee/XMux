...
if version.returncode != 0 or "xmux 1.0.3" not in version.stdout:
    sys.exit(10)
...
send = run_xmux(
    [
        ...
        "review",
        "--prompt",
        "CLAUDE_E2E_REQUEST: reply with HOOK-PONG-1.0.3",
        "--wait",
        "--timeout",
        "30",
        "--json",
    ...
    if answered_codex_request and not sent_codex_request and os.path.exists(CODEX_ACCEPTED):
        sent = run_xmux(
            [
                ...
                "E2E Claude to Codex",
                "--prompt",
                "CODEX_E2E_REQUEST: reply with CODEX-PONG-1.0.3",
                "--json",
            ],
            timeout=30,
        )
...
[[ "$version_output" == "xmux 1.0.3" ]] || {
  print -u2 "expected xmux 1.0.3, got: $version_output"
  exit 1
}
...
for _ in {1..80}; do
  ...
  [[ "$session_version" == "1.0.3" ]] && break
  sleep 0.25
done
[[ "$session_version" == "1.0.3" ]] || {
  print -u2 "expected tmux @xmux-version 1.0.3, got: ${session_version:-missing}"
  print_debug
  exit 1
}
